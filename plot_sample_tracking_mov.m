...
frames = read(v,[max(1,center_frame_k-frameOffset_color)...
    min(v.NumFrames,center_frame_k+frameOffset_color-1)]);
...
if useOverlay
    ...
    framesIR = read(vIR,[max(1,center_frame_k_IR-frameOffset_IR)...
        min(vIR.NumFrames,center_frame_k_IR+frameOffset_IR-1)]);
...
imH = image(frames(:,:,:,1));
ylim(ROI_rot.ylims+[-100 100])
xlim(ROI_rot.xlims+[-100 100])
axis square
%%
if saveMovie
...
for frame_k = 1:nFrames
    ...
    for batNum = batNums
        ...
        currentLoc = pd.pos2loc(batPos(frameIdx-frameOffset_color+frame_k+1,:),ROI_rot);
